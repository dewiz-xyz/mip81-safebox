 ...
 * @notice
 * - The `owner`(MakerDAO Governance) is in full control of how much and when it can send assets to `recipient`.
 *     - MakerDAO Governance could add other owners if required in the future (i.e.: automation of the Safebox balance).
 * - If MakerDAO governance ever executes an Emergency Shutdown, anyone can send assets to `recipient`.
 *     - This prevents assets being stuck in this contract when the governance smart contract is no longer operational.
...
contract Safebox {
    ...
    function withdraw(address token, uint256 amount) external {
        ...
        ERC20Like(token).transfer(recipient, amount);
